...
def reading_data(boson_file_number):
    ...
    boson_file = np.genfromtxt(boson_files[boson_file_number],
    ...
    for row_index, row in enumerate(boson_file_modified):
        if np.isnan(row[0]) is True or np.isnan(
                row[1]) is True or np.isnan(row[2]) is True:
            nan_row_indices.append(row_index)
    boson_file_modified = np.delete(boson_file_modified,
                                    nan_row_indices, axis=0)
    for i in [0,1,2]:
        boson_file_modified = boson_file_modified[
            boson_file_modified[:,i] >= 0]
...
def outlier_removal(datafile, mass, gamma_z):
    ...
    for row_index, _ in enumerate(datafile):
        ...
        if cross_section<predicted_cross_section-3*sigma:
            outlier_row_indices.append(row_index)
    datafile_modified = np.delete(datafile,
                                  outlier_row_indices,
                                  axis=0)
    return datafile_modified

def cross_section_function(energy, mass, gamma_z):
    return 0.3894e6*((12*np.pi/mass**2)*(energy*GAMMA_EE)**2/
                   ((energy**2-mass**2)**2+(mass*gamma_z)**2)) #nanobarns
...
def chi_squared(mass_width, energy, observed, uncertainty):
    ...
    return np.sum(((cross_section_function(energy, mass, width)
                    -observed)/uncertainty)**2)

def reduced_chi_squared(mass_width, energy, observed, uncertainty):
    degrees_of_freedom=len(observed)-2

    return chi_squared(mass_width,
                       energy,
                       observed,
                       uncertainty)/degrees_of_freedom
...
def mesh_arrays(x_array, y_array, energy, observed, uncertainty):
    ...
    for i,x_value in enumerate(x_array):
        for j,y_value in enumerate(y_array):
            chi_squared_value = (chi_squared((x_value,y_value),
                                             energy,
                                             observed,
                                             uncertainty))
...
def minimisation(mass_start, gamma_start, x_data, y_data, uncert):
    ...
    minimised_results = fmin(chi_squared, guesses,
                             args=(x_data, y_data, uncert),
...
def contour_plot(energy, observed, uncertainty, chi_square):
    ...
    mass_mesh, gamma_mesh, chi_squared_mesh = mesh_arrays(mass_array,
                                                          gamma_array,
                                                          energy,
                                                          observed,
                                                          uncertainty)
    ax2 = plt.figure().add_subplot(111)
    ax2.set_title(r'$\chi^2$ contours against $m_Z$ and $\Gamma_Z$.',
                                      fontsize=14)
    ax2.set_xlabel('$m_Z$', fontsize=14)
    ax2.set_ylabel(r'$\Gamma_Z$', fontsize=14)
    ax2.scatter(mass_min3, gamma_min3)
    plot = ax2.contour(mass_mesh,
                     gamma_mesh,
                     chi_squared_mesh,
                     levels= [chi_square+1,
                              chi_square+2.3,
    ...
    for index, label in enumerate(labels):
        ax2.collections[index].set_label(label)
    ax2.legend(loc='center left', bbox_to_anchor=(1, 0.5),
                                   fontsize=14)
    return plot

def uncertanties(energy, observed, uncertainty, chi_square):
    mass_array, gamma_array = array_gen(mass_min3, gamma_min3)
    contour = contour_plot(energy, observed, uncertainty, chi_square)
...
mass_min2, gamma_min2, cross_section_min2 = minimisation(MASS_GUESS,
                                                         GAMMA_Z_GUESS,
                                                         Energy1,
                                                         Cross_Section1,
...
mass_min3, gamma_min3, cross_section_min3 = minimisation(MASS_GUESS,
                                                         GAMMA_Z_GUESS,
                                                         Energy2,
                                                         Cross_Section2,
                                                         Cross_Section_errors2)

mass_uncertainty, gamma_uncertainty = uncertanties(Energy2,
                                              Cross_Section2, 
                                              Cross_Section_errors2,
                                              cross_section_min3)
...
ax.scatter(Energy2, Cross_Section2,
           marker='x', linewidth = 1,
           label='Data points')
ax.errorbar(Energy2, Cross_Section2, Cross_Section_errors2,
            ls='none', ecolor='green', linewidth=1.3,
...
Energy_Range = np.linspace(Energy_Minimum, Energy_Maximum,
                           1000)

ax.plot(Energy_Range,
        cross_section_function(Energy_Range,
                               mass_min3,
                               gamma_min3),
        color='r', label='Fit of data')
ax.legend(loc='upper left')

#contour plot from function
contour_plot(Energy2, Cross_Section2, Cross_Section_errors2,
             cross_section_min3)
...
print('The Gamma value is {0:.4g} +/- {3:.4f} and the mass is {1:.4g} +/- {4}.'
      ' The lifetime of the particle is {2:.3E}s.'
      .format(gamma_min3, mass_min3, lifetime(gamma_min3),
              mass_uncertainty, gamma_uncertainty))
print('The reduced chi^2 value is {0:.3f}'
      .format(reduced_chi_squared((mass_min3,gamma_min3),
                                  Energy2,Cross_Section2,
                                  Cross_Section_errors2)))